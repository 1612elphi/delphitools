...
export function buildPlugins(
	...
	if (strong) keys['Mod-b'] = toggleMark(strong);
	if (em) keys['Mod-i'] = toggleMark(em);
	if (code) keys['Mod-`'] = toggleMark(code);
	if (strikethrough) keys['Mod-Shift-x'] = toggleMark(strikethrough);
	...
	if (shiftTabChain.length)
		navKeys['Shift-Tab'] = chainCommands(...shiftTabChain);
