...
export class FootnoteView implements NodeView {
	...
	private open() {
		const tooltip = this.dom.appendChild(
	...
	update(node: PMNode): boolean {
		...
		if (this.innerView) {
			...
			const start = node.content.findDiffStart(
				state.doc.content,
			);
			if (start != null) {
				const diff = node.content.findDiffEnd(
					state.doc.content,
				);
				if (diff) {
					let { a: endA, b: endB } = diff;
					const overlap =
						start - Math.min(endA, endB);
					if (overlap > 0) {
						endA += overlap;
						endB += overlap;
					}
