...
const withInches = ({
	...
	const derivedWidthMm =
		widthMm ?? (widthIn != null ? widthIn * INCH_TO_MM : 0);
	const derivedHeightMm =
		heightMm ?? (heightIn != null ? heightIn * INCH_TO_MM : 0);
	const derivedWidthIn =
		widthIn ?? (widthMm != null ? widthMm / INCH_TO_MM : 0);
	const derivedHeightIn =
		heightIn ?? (heightMm != null ? heightMm / INCH_TO_MM : 0);
...
export function matchesNameSearch(size: PaperSize, query: string): boolean {
	...
	return (
		size.label.toLowerCase().includes(q) ||
...
export function findClosestSizes(
	...
	const withDistance = all.map((size) => {
		const normalDistance = calculateSizeDistance(
			size,
			targetWidthMm,
			targetHeightMm,
		);
		const rotatedDistance = calculateSizeDistance(
			size,
			targetHeightMm,
			targetWidthMm,
		);
