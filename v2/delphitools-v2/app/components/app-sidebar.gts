...
export default class AppSidebar extends Component {
  ...
  get collapsed() {
    return this.sidebar.state === 'collapsed' && !this.sidebar.isMobile;
  }
  ...
  <template>
    ...
    <aside
      ...
      data-open-mobile={{if this.sidebar.openMobile "true" "false"}}
    >
      <div class="dt-sidebar-header">
        <LinkTo @route="index" class="dt-brand">
          <img
            ...
            alt=""
            class="dt-brand-logo {{if this.pride 'pride-ring'}}"
          />
          <span class="dt-brand-text">
            <span class="dt-brand-name {{if this.pride 'pride-wordmark'}}">
              delphitools
            </span>
            <span class="dt-brand-sub">
              <span class="dt-brand-tagline {{if this.pride 'pride-tagline'}}">
                {{#if this.pride}}trans rights{{else}}indie tools{{/if}}
              </span>
              <span class="dt-brand-sha" title="Build commit">
                version:
                {{this.commitSha}}
              </span>
            </span>
      ...
      <nav aria-label="Main" class="dt-sidebar-nav">
        {{#unless this.query}}
          <div class="dt-nav-group">
            <NavTip @label="Home" @show={{this.collapsed}}>
              <LinkTo @route="index" class="dt-nav-link">
                <Icon @name="home" />
                <span class="dt-nav-label">Home</span>
              </LinkTo>
            </NavTip>
            {{! the editor is more than a tool, so it sits with Home }}
            <NavTip @label="Substrata" @show={{this.collapsed}}>
              <LinkTo @route="editor" class="dt-nav-link">
                <Icon @name="brush" />
                <span class="dt-nav-label">Substrata</span>
              </LinkTo>
            </NavTip>
          </div>
        {{/unless}}
        ...
        {{#if this.empty}}
          <output class="dt-nav-empty" aria-live="polite">No tools found</output>
        {{/if}}

        {{#if this.featured.length}}
          <div class="dt-nav-group">
            <h2 class="dt-nav-group-label">
              <Icon @name="star" class="dt-star" />
              Greatest Hits
            </h2>
            {{#each this.featured as |tool|}}
              <NavTip @label={{tool.name}} @show={{this.collapsed}}>
                <LinkTo
                  @route="tools.tool"
                  @model={{tool.id}}
                ...
                >
                  <Icon @name={{tool.icon}} />
                  <span class="dt-nav-label">{{tool.name}}</span>
                </LinkTo>
              </NavTip>
            {{/each}}
          </div>
        ...
        {{#each this.categories as |category|}}
          <div class="dt-nav-group">
            <h2 class="dt-nav-group-label">{{category.name}}</h2>
      ...
      <div class="dt-sidebar-footer">
        <Dialog as |d|>
          ...
          >
            ...
            <Icon @name="info" class="dt-about-icon" />
            <span class="dt-sr-only">About delphitools</span>
          </button>
