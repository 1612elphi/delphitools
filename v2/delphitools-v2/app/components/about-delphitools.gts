...
const BUILT_WITH = [
  ...
  { name: 'Embroider', url: 'https://github.com/embroider-build/embroider' },
...
const AboutDelphitoolsBody: TOC<{ Element: HTMLDivElement }> = <template>
  <div class="dt-about-body" ...attributes>
    <div class="dt-about-lede">
      <p>
        delphitools is a collection of small, focused utilities that respect
        your privacy and work entirely in your browser. No data leaves your
        machine, no accounts required, no tracking. Just tools that do what they
        say.
      </p>
      <p>
        I love the web. The classic, real web full of weird things. And that web
        is out there. You just have to find it. And sometimes, you have to make
        it yourself.
      </p>
    </div>
    ...
    <div class="dt-about-cols">
      <div>
        <h3>Made by</h3>
        <p>
          <a href="https://rmv.fyi" target="_blank" rel="noopener noreferrer">
            delphi<span class="dt-sr-only">(opens in new tab)</span>
          </a>
        </p>
      </div>
      <div>
        ...
        <p>
          ...
          >
            1612elphi/delphitools<span class="dt-sr-only">(opens in new tab)</span>
          </a>
        </p>
      </div>
    </div>
    ...
    <div class="dt-about-block">
      ...
      <div class="dt-chips">
        {{#each CONTRIBUTORS as |person|}}
          <a href={{person.url}} target="_blank" rel="noopener noreferrer">
            {{person.name}}<span class="dt-sr-only">(opens in new tab)</span>
          </a>
        {{/each}}
      </div>
      <p class="dt-about-note">
        <a
          ...
          rel="noopener noreferrer"
        >
          Behind the scenes of delphitools<span class="dt-sr-only">(opens in new
            tab)</span>
        </a>
      </p>
    </div>
    ...
    <div class="dt-about-block">
      ...
      <p class="dt-about-note">
        Folks who, instead of donating to delphitools, gave to Wikipedia or the
        EFF on its behalf.
      </p>
      <div class="dt-chips">
        {{#each DONORS as |donor|}}
          <a href={{donor.url}} target="_blank" rel="noopener noreferrer">
            {{donor.name}}
            ·
            {{donor.org}}<span class="dt-sr-only">(opens in new tab)</span>
          </a>
        {{/each}}
      </div>
    </div>
      ...
      <div class="dt-chips">
        {{#each BUILT_WITH as |lib|}}
          <a href={{lib.url}} target="_blank" rel="noopener noreferrer">
            {{lib.name}}<span class="dt-sr-only">(opens in new tab)</span>
          </a>
        {{/each}}
      </div>
      <p class="dt-about-note">
        ...
        <a
          ...
          rel="noopener noreferrer"
        >
          many more open source libraries<span class="dt-sr-only">(opens in new
            tab)</span>
        </a>.
      </p>
    </div>
