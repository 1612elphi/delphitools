...
export default class AppHeader extends Component {
  ...
  get toolId(): string | undefined {
    if (this.router.currentRouteName !== 'tools.tool') return undefined;
    return this.router.currentRoute?.params?.['tool_id'] as string | undefined;
  }
    ...
    <header class="dt-header">
      ...
      {{#if this.tool}}
        <span class="dt-header-title">
          <Icon @name={{this.tool.icon}} class="dt-header-icon" />
          <h1>{{this.tool.name}}</h1>
          {{#if this.category}}
            <span class="dt-badge">{{this.category.name}}</span>
          {{/if}}
        </span>
      {{else if this.isHome}}
        <span class="dt-header-title">
          <Icon @name="home" class="dt-header-icon" />
          <h1>Home</h1>
        </span>
      {{else}}
        <span class="dt-header-title">
          <img src="/delphi.png" alt="" class="dt-header-icon" />
          <h1>delphitools</h1>
        </span>
      {{/if}}
