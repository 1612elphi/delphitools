...
const ux = await page.evaluate(() => {
  ...
  const units = [...(dock?.querySelectorAll(":scope .pointer-events-auto.shadow-lg") ?? [])]
    .filter((el) => !el.querySelector(".pointer-events-auto.shadow-lg")); // leaf boxes
...
// rail counted separately → leaf + >=4
...
const starVisible = await page.evaluate(() => {
  // only open bloom's children interactive
...
if (gripRect) {
  ...
  await page.mouse.move(gripRect.x + 40, gripRect.y - 40, { steps: 4 }); // past threshold
  await sleep(200);
