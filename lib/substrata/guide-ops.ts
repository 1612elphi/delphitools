...
function write(mutator: (doc: SubstrataDoc) => SubstrataDoc, transient?: boolean): void {
...
export function setGuidePos(id: string, pos: number, opts?: { transient?: boolean }): void {
  write(
    (doc) => ({
      ...doc,
      guides: doc.guides.map((g) => (g.id === id ? { ...g, pos } : g)),
      updatedAt: Date.now(),
    }),
    opts?.transient,
  );
