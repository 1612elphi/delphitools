...
export function starPoints(points: number, outerRadius: number, innerRadius: number): Pt[] {
 ...
 * shorter than 2r).
 */
export function shapeDims(params: ShapeParams): { width: number; height: number } {
  switch (params.shape) {
    case "rectangle":
