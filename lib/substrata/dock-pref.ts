...
const isFloatPos = (v: unknown): v is FloatPos =>
  !!v && typeof v === "object" && typeof (v as FloatPos).x === "number" && typeof (v as FloatPos).y === "number";
...
export function hydrateLayoutPrefs(): void {
  ...
  if (rawFloat && typeof rawFloat === "object") {
    ...
    for (const id of MODULE_IDS) if (isFloatPos(rawFloat[id])) merged[id] = rawFloat[id] as FloatPos;
    floatPos = merged;
  }
  ...
  if (rawClamp && typeof rawClamp === "object") {
    ...
    for (const id of MODULE_IDS) if (typeof rawClamp[id] === "boolean") merged[id] = rawClamp[id] as boolean;
    clamped = merged;
  }
...
export function floatModule(id: ModuleId, pos: FloatPos): void {
  ...
  floatPos = { ...floatPos, [id]: { x: Math.round(pos.x), y: Math.round(pos.y) } };
