...
export function maxTextureSize(): number {
  ...
  if (typeof document === "undefined") return (cachedMax = FALLBACK_MAX_TEXTURE);
  try {
    ...
    cachedMax = typeof max === "number" && max > 0 ? max : FALLBACK_MAX_TEXTURE;
  } catch {
 ...
 */
export function clampToCap(width: number, height: number, cap = workingRasterCap()): ClampResult {
