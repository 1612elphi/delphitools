...
export type MenuState =
  ...
  | { kind: "canvas"; x: number; y: number; scene: { x: number; y: number } };
...
export function openLayerMenu(x: number, y: number, ids: readonly string[]): void {
  if (ids.length === 0) return;
  state = { kind: "layer", x, y, ids };
  emit();
}

export function openCanvasMenu(x: number, y: number, scene: { x: number; y: number }): void {
