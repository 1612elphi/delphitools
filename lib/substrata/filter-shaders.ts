...
import type { T2DPipelineState, TWebGLPipelineState, TWebGLUniformLocationMap } from "fabric";
...
const lumaOf = (r: number, g: number, b: number): number => r * LUMA[0] + g * LUMA[1] + b * LUMA[2];
...
type LevelsProps = { inBlack: number; inWhite: number; gamma: number; outBlack: number; outWhite: number };

export class SubstrataLevels extends BaseFilter<"SubstrataLevels", LevelsProps> {
  ...
  static defaults: LevelsProps = { inBlack: 0, inWhite: 1, gamma: 1, outBlack: 0, outWhite: 1 };
  static uniformLocations = ["uInBlack", "uInWhite", "uGamma", "uOutBlack", "uOutWhite"];
  ...
  applyTo2d({ imageData: { data } }: T2DPipelineState): void {
    ...
    for (let i = 0; i < 256; i++) {
      const c = Math.pow(clamp01((i / 255 - this.inBlack) / range), 1 / this.gamma);
      lut[i] = 255 * (this.outBlack + c * (this.outWhite - this.outBlack));
    }
    for (let i = 0; i < data.length; i += 4) {
      data[i] = lut[data[i]];
  ...
  sendUniformData(gl: WebGLRenderingContext, u: TWebGLUniformLocationMap): void {
  ...
  isNeutralState(): boolean {
    return this.inBlack === 0 && this.inWhite === 1 && this.gamma === 1 && this.outBlack === 0 && this.outWhite === 1;
  }
}
...
export class SubstrataThreshold extends BaseFilter<"SubstrataThreshold", ThresholdProps> {
  ...
  applyTo2d({ imageData: { data } }: T2DPipelineState): void {
    ...
    for (let i = 0; i < data.length; i += 4) {
      const v = lumaOf(data[i], data[i + 1], data[i + 2]) >= cut ? 255 : 0;
...
export class SubstrataPosterise extends BaseFilter<"SubstrataPosterise", PosteriseProps> {
  ...
  applyTo2d({ imageData: { data } }: T2DPipelineState): void {
    ...
    const lut = new Uint8ClampedArray(256);
    for (let i = 0; i < 256; i++) lut[i] = (Math.round((i / 255) * n) / n) * 255;
    for (let i = 0; i < data.length; i += 4) {
      data[i] = lut[data[i]];
      data[i + 1] = lut[data[i + 1]];
      data[i + 2] = lut[data[i + 2]];
...
type VignetteProps = { amount: number; midpoint: number; roundness: number; feather: number; colour: Vec3 };

export class SubstrataVignette extends BaseFilter<"SubstrataVignette", VignetteProps> {
  ...
  static defaults: VignetteProps = { amount: 0.5, midpoint: 0.5, roundness: 0, feather: 0.5, colour: [0, 0, 0] };
  static uniformLocations = ["uAmount", "uMidpoint", "uRoundness", "uFeather", "uColour"];
  ...
  applyTo2d({ imageData: { data, width, height } }: T2DPipelineState): void {
    ...
    const [cr, cg, cb] = this.colour.map((c) => c * 255);
    for (let y = 0; y < height; y++) {
      const py = y / height - 0.5;
      for (let x = 0; x < width; x++) {
        const px = (x / width - 0.5) * xScale;
        const mix = smoothstep(edge, edge + feather, Math.sqrt(px * px + py * py) * 2) * this.amount;
        if (mix === 0) continue;
        const i = (y * width + x) * 4;
...
export class SubstrataDuotone extends BaseFilter<"SubstrataDuotone", DuotoneProps> {
  declare shadow: Vec3;
  declare highlight: Vec3;
  declare midpoint: number;
  static type = "SubstrataDuotone";
  static defaults: DuotoneProps = { shadow: [0, 0, 0], highlight: [1, 1, 1], midpoint: 0.5 };
  ...
  private gammaMap(): number {
    return Math.log(0.5) / Math.log(Math.min(0.99, Math.max(0.01, this.midpoint)));
  }
  ...
  applyTo2d({ imageData: { data } }: T2DPipelineState): void {
    ...
    for (let i = 0; i < data.length; i += 4) {
      const t = Math.pow(lumaOf(data[i], data[i + 1], data[i + 2]) / 255, g);
...
type ColourBalanceProps = { cyanRed: number; magentaGreen: number; yellowBlue: number };

export class SubstrataColourBalance extends BaseFilter<"SubstrataColourBalance", ColourBalanceProps> {
  declare cyanRed: number;
  declare magentaGreen: number;
  declare yellowBlue: number;
  static type = "SubstrataColourBalance";
  static defaults: ColourBalanceProps = { cyanRed: 0, magentaGreen: 0, yellowBlue: 0 };
  ...
  private shift(): Vec3 {
    ...
    return [this.cyanRed * k, this.magentaGreen * k, this.yellowBlue * k];
  }

  applyTo2d({ imageData: { data } }: T2DPipelineState): void {
    const [dr, dg, db] = this.shift().map((v) => v * 255);
    for (let i = 0; i < data.length; i += 4) {
  ...
  isNeutralState(): boolean {
    return this.cyanRed === 0 && this.magentaGreen === 0 && this.yellowBlue === 0;
  }
}

// ── Film sim (the LUT family's engine: a lift/gamma/gain + saturation grade) ──

type FilmSimProps = { lift: Vec3; gain: Vec3; gamma: Vec3; sat: number; intensity: number };

export class SubstrataFilmSim extends BaseFilter<"SubstrataFilmSim", FilmSimProps> {
  declare lift: Vec3;
  declare gain: Vec3;
  declare gamma: Vec3;
  declare sat: number;
  ...
  static defaults: FilmSimProps = { lift: [0, 0, 0], gain: [1, 1, 1], gamma: [1, 1, 1], sat: 1, intensity: 1 };
  static uniformLocations = ["uLift", "uGain", "uGamma", "uSat", "uIntensity"];
  ...
  applyTo2d({ imageData: { data } }: T2DPipelineState): void {
    ...
    applyGradeToImageData(data, { lift, gain, gamma, sat }, intensity);
  }
...
export class SubstrataLut extends BaseFilter<"SubstrataLut", LutProps> {
  ...
  applyToWebGL(options: TWebGLPipelineState): void {
    ...
    const texture = options.filterBackend.getCachedTexture(this.lutKey, this.lut.strip, gl.LINEAR);
