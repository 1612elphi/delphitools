...
export function jxlAvailable(): boolean {
  return typeof Worker !== "undefined" && typeof window !== "undefined" && window.isSecureContext === true;
}
...
function getJxlWorker(): Worker {
  if (!worker) {
    ...
    worker.onmessage = (event: MessageEvent) => {
      ...
      if (!p) return;
      pending.delete(id);
      if (ok && bytes) p.resolve(new Blob([bytes], { type: "image/jxl" }));
      else p.reject(new Error(error ?? "JXL encoding failed"));
    };
    worker.onerror = (event) => {
      // A dead worker fails everything in flight; the next encode respawns it.
      for (const p of pending.values()) p.reject(new Error(event.message || "JXL worker error"));
      pending.clear();
      worker?.terminate();
      worker = null;
    };
...
export async function encodeCanvas(
  ...
  const blob = await canvasToBlob(canvas, meta.mime, meta.lossy ? quality : undefined);
  if (blob.type && blob.type !== meta.mime) {
    throw new Error(`Encoder produced ${blob.type} instead of ${meta.mime}`);
  }
  return blob;
}
