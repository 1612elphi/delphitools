...
// are flat exports).
const { Blur, Brightness, ColorMatrix, Contrast, Convolute, Gamma, Grayscale, HueRotation, Invert, Noise, Pixelate, Saturation, Sepia, Vibrance } = fabricFilters;
 ...
 */
const FACTORIES: Record<string, (p: Params, size: { width: number; height: number }) => FabricFilter | null> = {
  ...
  brightness: (p) => new Brightness({ brightness: num(p, "amount") / 200 }),
  contrast: (p) => new Contrast({ contrast: num(p, "amount") / 200 }),
  saturation: (p) => new Saturation({ saturation: num(p, "amount") / 100 }),
  vibrance: (p) => new Vibrance({ vibrance: num(p, "amount") / 100 }),
  "hue-rotate": (p) => new HueRotation({ rotation: num(p, "angle") / 180 }),
  ...
  sepia: () => new Sepia(),
  invert: () => new Invert(),
  gamma: (p) => new Gamma({ gamma: [num(p, "red") || 1, num(p, "green") || 1, num(p, "blue") || 1] }),
  ...
  "gaussian-blur": (p, size) => new Blur({ blur: Math.min(1, num(p, "radius") / (0.12 * Math.min(size.width, size.height))) }),
  // Amount lerps the kernel identity→classic (t=1 is the textbook kernel).
  sharpen: (p) => {
    const t = num(p, "amount") / 100;
    return new Convolute({ matrix: [0, -t, 0, -t, 1 + 4 * t, -t, 0, -t, 0] });
  },
  emboss: (p) => {
    const t = num(p, "amount") / 100;
    return new Convolute({ matrix: [t, t, t, t, 1 - 0.3 * t, -t, -t, -t, -t] });
  },
  "edge-detect": kernel([-1, -1, -1, -1, 8, -1, -1, -1, -1]),
  noise: (p) => {
    const noise = (num(p, "amount") / 100) * 255;
    return p.mode === "colour" ? new ColourNoise({ noise }) : new Noise({ noise });
  },
  pixelate: (p) => new Pixelate({ blocksize: Math.max(2, Math.round(num(p, "blockSize"))) }),
  ...
  threshold: (p) => new SubstrataThreshold({ level: num(p, "level") / 255 }),
  posterize: (p) => new SubstrataPosterise({ levels: Math.min(32, Math.max(2, Math.round(num(p, "levels")))) }),
  vignette: (p) =>
  ...
  "film-sim": (p) => {
    ...
    if (isLutLook(preset)) {
      ...
      return new SubstrataLut({ lut, lutKey: `substrata_${preset}`, intensity });
    }
    return null;
  },
};
...
export function buildFabricFilters(
  ...
  for (const f of stack) {
    ...
    const built = make(def ? { ...defaultParams(def.params), ...f.params } : f.params, size);
