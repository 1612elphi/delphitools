...
function segmentSentences(text: string): [number, number][] {
	const Segmenter = (Intl as unknown as { Segmenter?: SegmenterCtor })
		.Segmenter;
	if (Segmenter) {
		const out: [number, number][] = [];
		for (const s of new Segmenter('en', {
...
function buildDecorations(
	...
	if (
		activeStart >= 0 &&
		(settings.highlightParagraph || settings.dimInactive)
	) {
		const activeEnd = $from.after(1);
		if (settings.highlightParagraph) {
			decos.push(
				Decoration.node(activeStart, activeEnd, {
					class: 'pm-para-active',
				}),
			);
		}
	...
	if (settings.highlightSentence && $from.parent.isTextblock) {
		...
		const from = Math.max(
			blockStart,
			Math.min(blockStart + s, blockEnd),
		);
		const to = Math.max(
			blockStart,
			Math.min(blockStart + e, blockEnd),
		);
		if (to > from)
			decos.push(
				Decoration.inline(from, to, {
					class: 'pm-sentence-active',
				}),
			);
	}
...
export function focusPlugin(
	initial: EditorSettings = DEFAULT_SETTINGS,
): Plugin<EditorSettings> {
	return new Plugin<EditorSettings>({
		key: focusKey,
		state: {
			init: () => initial,
			apply: (tr, value) =>
				(tr.getMeta(focusKey) as
					EditorSettings | undefined) ?? value,
		},
		props: {
			decorations(state) {
				const settings =
					focusKey.getState(state) ??
					DEFAULT_SETTINGS;
