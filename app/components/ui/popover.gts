...
// restyled onto Crayon: floating-ui positioning, the context plumbing and the
// click-outside/escape handling are kept as-is, the Tailwind class strings
// become .dt-popover* hooks styled in app/styles/app.scss.
...
type PopoverTriggerSignature =
	PopoverTriggerSignatureAsChild | PopoverTriggerSignatureAsRoot;

class PopoverTrigger extends Component<PopoverTriggerSignature> {
	@consume(PopoverContext)
	context!: ContextRegistry[typeof PopoverContext];
	...
	<template>
		...
		{{else}}
			<button
				class="dt-popover-trigger {{@class}}"
...
class PopoverContent extends Component<PopoverContentSignature> {
	...
	positionContent = modifier((element: HTMLElement) => {
		...
		const placement = placementMap[`${side}-${align}`] || 'bottom';
	...
	<template>
		{{#if this.context.isRendered}}
			{{#in-element
				this.destinationElement insertBefore=null
			}}
				<div
					class="dt-popover {{@class}}"
