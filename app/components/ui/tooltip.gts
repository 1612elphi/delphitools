...
// restyled onto Crayon: the floating-ui positioning, context plumbing and
...
class TooltipTrigger extends Component<TooltipTriggerSignature> {
	@consume(TooltipContext)
	context!: ContextRegistry[typeof TooltipContext];
		...
		const update = () => {
			void computePosition(triggerElement, element, {
				...
				({
					...
					this.x = x;
					this.y = y;
					this.actualSide = actualPlacement.split(
						'-',
					)[0] as
						| 'top'
						| 'right'
